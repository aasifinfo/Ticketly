    ...
    async function poll() {
        ...
        try {
            ...
            if (data.status === 'failed') {
                ...
                setTimeout(() => { window.location.href = '{{ route('checkout.show', $reservation->token) }}'; }, 2000);
                return;
            }
            if (data.status === 'expired') {
                document.getElementById('status-msg').textContent = data.message || 'Your hold expired. Redirecting to event page...';
                setTimeout(() => {
                    window.location.href = data.redirect_to || '{{ route('events.show', $reservation->event->slug) }}';
                }, 1800);
                return;
            }
