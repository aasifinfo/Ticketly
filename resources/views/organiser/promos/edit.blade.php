...
<div class="max-w-lg">
  <div class="flex items-center justify-between mb-5">
    ...
    <a href="{{ route('organiser.promos.show', $promo->id) }}" class="text-xs font-semibold text-blue-300 border border-blue-600/40 px-3 py-2 rounded-lg hover:bg-blue-900/20 transition-colors">View</a>
  </div>
  ...
  <div class="bg-gray-900 border border-gray-800 rounded-2xl p-8">
    ...
    <form action="{{ route('organiser.promos.update', $promo->id) }}" method="POST" class="space-y-4">
      @csrf
      @method('PUT')

      <div>
        <label class="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">Code *</label>
        <input type="text" name="code" value="{{ old('code', $promo->code) }}" required maxlength="30"
               class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 uppercase tracking-widest font-mono"
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <select name="type" id="promo-type" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500" onchange="updateValueLabel()">
            ...
            <option value="fixed" {{ old('type', $promo->type) === 'fixed' ? 'selected' : '' }}>Fixed Amount ({{ ticketly_currency_symbol() }})</option>
          </select>
        </div>
        <div>
          ...
          <input type="number" name="value" value="{{ old('value', $promo->value) }}" required step="0.01" min="0.01"
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="number" name="max_discount" value="{{ old('max_discount', $promo->max_discount) }}" step="0.01" min="0"
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" name="expires_at" value="{{ old('expires_at', optional($promo->expires_at)->format('Y-m-d')) }}"
        ...
        <div>
          ...
          <select name="event_id" class="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500">
            ...
            <option value="{{ $event->id }}" {{ (string) old('event_id', $promo->event_id) === (string) $event->id ? 'selected' : '' }}>{{ Str::limit($event->title, 35) }}</option>
            @endforeach
          </select>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $promo->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-600 bg-gray-700 text-indigo-600">
      ...
      <div class="flex gap-3 pt-2">
        ...
        <a href="{{ route('organiser.promos.show', $promo->id) }}" class="px-5 py-3 text-sm font-semibold text-gray-400 bg-gray-800 hover:bg-gray-700 rounded-xl transition-colors">Cancel</a>
      </div>
    </form>
  </div>
</div>
